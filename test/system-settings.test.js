...
test("global proxies setting survives a local settings update and read", () => {
  const directory = mkdtempSync(join(tmpdir(), "deepseek2api-settings-"));
  try {
    mkdirSync(join(directory, "data"), { recursive: true });
    const script = [
      `const settings = await import(${JSON.stringify(settingsModuleUrl)});`,
      "settings.updateSystemSettings({ globalProxies: 'http://proxy1:8080\\nhttp://proxy2:8080' });",
      "console.log(JSON.stringify(settings.getPublicSystemSettings()));"
    ].join("\n");
    const result = spawnSync(process.execPath, ["--input-type=module", "--eval", script], {
      cwd: directory,
    ...
    assert.equal(result.status, 0, result.stderr);
    const payload = JSON.parse(result.stdout.trim());
    assert.deepEqual(payload.globalProxies, ["http://proxy1:8080", "http://proxy2:8080"]);
  } finally {
    rmSync(directory, { recursive: true, force: true });
  }
});
